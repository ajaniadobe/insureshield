...
/**
 * Transformer: InsureShield Dynamic Media / Scene7 images.
 *
 * The source uses Adobe Scene7 IS/Image URLs, e.g.
 *   https://ups.scene7.com/is/image/upsprod/newhomepagehero?$UPSer_Preset$&wid=1440&hei=640&fit=constrain
 * The literal `$UPSer_Preset$` template parameter must survive to the
 * client, so we round-trip the DM URL through markdown by rewriting each
 * DM <img> into a carrier anchor (href for unlinked, title for linked).
 * The companion auto-block in scripts/scripts.js rebuilds responsive
 * <picture> markup from these anchors at render time.
 *
 * Runs in afterTransform ONLY: block parsers run between the two hooks and
 * extract <img> references into block cells (cards / story carousel). If we
 * rewrote imgs to anchors in beforeTransform the parsers would find no img
 * and emit empty image cells.
 *
 * Helpers below are copied verbatim from the canonical
 * references/dm-scene7-helpers.js (transformer subset) — keep byte-identical.
 */
...
export default function transform(hookName, element, payload) {
  ...
  element.querySelectorAll('img').forEach((img) => {
    ...
    // via linkTextToAlt() so screen readers correctly skip decorative images.
